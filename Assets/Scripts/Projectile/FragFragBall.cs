...
public class FragFragBall : Projectile ///Team members that contributed to this script: Ian Bunnell
{
    ...
    public override void OnDeath(bool OutBoundDeath)
    {
        if(!OutBoundDeath)
        {
            ...
            for (int i = 0; i < TotalFrags; i++)
            {
                ...
                NewProjectile(ProjectileID.FragBall, transform.position + velo.normalized, Quaternion.identity, 
